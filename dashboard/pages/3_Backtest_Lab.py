...
if st.sidebar.button("🚀 Run Backtest", type="primary"):
    if not model_files:
        ...
        # Force fresh fetch by bypassing default cache if dates are new
        # We pass 'cache_mode' if supported or just ensure parameters force refresh
        # load_test_data is cached by streamlit.
        
        ascii_loader(3, "CONNECTING TO DATA FEED")
        try:
             # We use the cached function but it relies on DataLoader
             prices, returns, features = load_test_data(start_date, end_date)
             ascii_loader(6, "COMPUTING TENSORS")
             
             result, signal_df, error = run_backtest_logic(model_path, prices, returns, features, start_date, end_date)
             
             ascii_loader(9, "FINALIZING METRICS")
             loading_placeholder.empty() # Clear
             
             if error:
                st.error(f"Backtest Failed: {error}")
                if "No data" in error:
                    st.warning("Try adjusting the 'Start Date' to earlier (e.g., 2024-01-01) or check if yfinance has 2025 data available.")
             else:
                ...
                # Positions is already a DataFrame
                ...
                # Dropdown to select asset for inspection
                ...
                # Create trades DataFrame from position changes
                trades_df = create_trades_df_from_positions(
                    positions_df=result.positions,
                    prices_df=prices.loc[result.positions.index],
                    signals_df=signal_df.reindex(result.positions.index) if signal_df is not None else None
                )
                ...
                st.caption("💡 **Interpretation:** If price exits the shaded cone, it indicates a regime shift or unexpected shock.")
